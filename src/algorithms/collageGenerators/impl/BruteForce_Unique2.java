...
public class BruteForce_Unique2 {
	...
	public BruteForce_Unique2(ResultXML result, ImageRefList refList){
	...
	public void loop(){
		...
		for(int cell = 0; cell<nCells; cell++){
			...
			//Add all currently available to all sets.
			//The recursive parent will remove the appropriate IDs.
			available[cell].images.addAll(keyList);
		}
	...
	private boolean recursionLoop(int cell){
		...
		if(cell!=0)available[cell-1].images.clear();
		if(cell!=0)available[cell-1].images.addAll(available[cell].images);
		for(int imgID : available[cell].images){
			if(cell!=0)available[cell-1].images.remove(imgID);
			...
			if(cell!=0)available[cell-1].images.add(imgID);
		}
