...
public class BruteForce_Unique {
	...
	public BruteForce_Unique(ResultXML result, ImageRefList refList){
	...
	//(Note that the constructor ensures that there are enough images to create a unique solution)
		...
		recursionLoop(result.grid.size.w*result.grid.size.h-1); //this method computes the best result (until termination)
	...
	private boolean recursionLoop(int cell){
		...
		Set<Integer> availableImagesLocal = new HashSet<Integer>();
		availableImagesLocal.addAll(this.availableImages);
		for(int imgID : availableImagesLocal){
		...
		availableImagesLocal.clear(); //dispose
